...
class PluginBrowserNew(Screen):
	...
	def setIcons(self):
		for x,elem in enumerate(self.plugins):
			x += 1
			icon = elem[3] or LoadPixmap(resolveFilename(SCOPE_GUISKIN, "icons/plugin.png"))
	...
	def openExtensionmanager(self):
		if fileExists(resolveFilename(SCOPE_PLUGIN, "SystemPlugins/SoftwareManager/plugin.py")):
			try:
				from Plugins.SystemPlugins.SoftwareManager.plugin import PluginManager
			except ImportError:
				self.session.open(MessageBox, _("The software management extension is not installed!\nPlease install it."), type=MessageBox.TYPE_INFO, timeout=10)
