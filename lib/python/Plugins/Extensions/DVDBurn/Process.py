	...
	def processOutputLine(self, line):
		...
		if line.startswith(MSG_NEW_FILE):
			...
			try:
				...
				print("[DVDBurn] ERROR: couldn't detect Audio PID (projectx too old?)")

	def haveNewFile(self, file):
		print("[DVDBurn] produced file:", file, self.currentPID)
	...
	def processOutputLine(self, line):
		...
		if line[8:14] == "done, ":
			...
			if line.find("No space left on device") != -1:
				...
				print("[DVDBurn] *sigh* this is a known bug. we're simply gonna assume everything is fine.")
				self.postconditions = []
